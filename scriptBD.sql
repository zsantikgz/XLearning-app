...
ALTER TABLE EVENTOS
ADD COLUMN URL_EVENTO VARCHAR(255);
...
ADD COLUMN DIRIGIDO_A VARCHAR(20) DEFAULT 'TODOS' CHECK (DIRIGIDO_A IN ('ESTUDIANTES', 'PROFESORES', 'TODOS'));

ALTER TABLE ANUNCIOS
ADD COLUMN IMPORTANCIA VARCHAR(20) DEFAULT 'NORMAL' CHECK (IMPORTANCIA IN ('IMPORTANTE', 'NORMAL'));
...
-- 17 TABLA FELICITACIONES USUARIOS (CUMPLEAÑOS)
CREATE TABLE FELICITACIONES_USUARIOS (
  ID_FELICITACIONES SERIAL PRIMARY KEY,
  ID_USUARIO INT NOT NULL,
  FECHA_CUMPLE DATE NOT NULL,
  URL_VIDEO TEXT NOT NULL,
  MENSAJE TEXT DEFAULT '¡Feliz cumpleaños!',
...
-- VALIDAR FELICITACION UNICA POR USUARIO
CREATE OR REPLACE FUNCTION verificar_felicitacion_unica()
RETURNS TRIGGER AS $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM FELICITACIONES_USUARIOS 
        WHERE ID_USUARIO = NEW.ID_USUARIO
    ) THEN
        RAISE EXCEPTION 'Ya existe una felicitación para este usuario.';
    END IF;
...
-- TRIGGER PARA VERIFICAR FELICITACION UNICA
CREATE TRIGGER trg_verificar_felicitacion
BEFORE INSERT ON FELICITACIONES_USUARIOS
FOR EACH ROW
EXECUTE FUNCTION verificar_felicitacion_unica();
